...
const JobUpdateForm = (props: any) => {
  const { record, thenSuccess } = props;
  const jobDrawer = useRef<any>();

  const JobUpdateForm = createForm({
    validateFirst: true,
    initialValues: record,
  });

  const { run: fetchUpdateJob } = useRequest(updateJob, {
    manual: true,
    onSuccess: () => {
    ...
    onFinally() {
      JobUpdateForm.loading = false;
    },
  });

  const handelSubmit = async (field: any) => {
    JobUpdateForm.loading = true;
    ...
    <Button
      ...
      onClick={() => {
        jobDrawer.current = FormDrawer(
          { maskClosable: true, footer: false, title: "修改岗位", width: 600 },
          () => (
            <div
              style={{
                display: "flex",
